...
<div class="stats-bar">
  <div class="stat-card total"><div class="num">{{ total }}</div><div class="lbl">Total Patients</div></div>
  <div class="stat-card pend"><div class="num">{{ pending }}</div><div class="lbl">Pending</div></div>
  <div class="stat-card rev"><div class="num">{{ reviewed }}</div><div class="lbl">Reviewed</div></div>
</div>
...
<div id="emptyMsg" style="display:none;text-align:center;padding:40px;color:#888;"><h5>No patients found.</h5></div>
</div>

{% if role=='technician' %}
<button class="chat-fab" onclick="toggleChat()" id="chatFab">🤖<span class="notif" id="notifBadge">1</span></button>
<div class="chat-popup" id="chatPopup">
  <div class="chat-header">
    <div><h6>🤖 TeleRad Assistant</h6><small>Machine & radiology AI guide</small></div>
    <button class="close-btn" onclick="toggleChat()">✕</button>
  </div>
  <div class="chat-messages" id="chatMessages">
    <div><div class="msg-label">🤖 Assistant</div>
...
What do you need help with today?</div></div>
  </div>
...
function filterCards(type){
  ...
  document.querySelectorAll('.patient-card').forEach(card=>{
    ...
    const show=type==='all'||(type==='pending'&&s==='Pending')||(type==='reviewed'&&s!=='Pending');
...
async function sendMessage(){
  ...
  input.value="";sendBtn.textContent="...";sendBtn.disabled=true;
  messages.scrollTop=messages.scrollHeight;
  const tid="t"+Date.now();
  messages.innerHTML+=`<div id="${tid}"><div class="msg-label">🤖 Assistant</div><div class="bot-msg" style="color:#8b949e;">Thinking...</div></div>`;
  messages.scrollTop=messages.scrollHeight;
  try{
    const resp=await fetch("/dashboard_chat",{method:"POST",headers:{"Content-Type":"application/json"},body:JSON.stringify({message:msg})});
  ...
  }catch(e){document.getElementById(tid).remove();messages.innerHTML+=`<div class="bot-msg" style="color:#ff6b6b;">⚠️ Error. Try again.</div>`;}
  sendBtn.textContent="➤";sendBtn.disabled=false;messages.scrollTop=messages.scrollHeight;
}
</script>
</body>
</html>
...
{% if has_conf %}
  ...
  {% if conf >= 90 %}{% set bc="#1e8449"%}{% set bg="#eafaf1"%}{% set bar="#27ae60"%}{% set icon="🟢"%}{% set lvl="High Confidence"%}
  {% elif conf >= 70 %}{% set bc="#d68910"%}{% set bg="#fef9e7"%}{% set bar="#f39c12"%}{% set icon="🟡"%}{% set lvl="Moderate Confidence"%}
  {% else %}{% set bc="#c0392b"%}{% set bg="#fdedec"%}{% set bar="#e74c3c"%}{% set icon="🔴"%}{% set lvl="Low Confidence"%}{% endif %}
<div style="background:{{ bg }};border:1.5px solid {{ bc }};border-radius:12px;padding:14px 18px;margin-bottom:8px;">
  <div style="display:flex;align-items:center;justify-content:space-between;flex-wrap:wrap;gap:8px;">
    <div><span style="font-size:18px;">{{ icon }}</span><b style="color:{{ bc }};font-size:15px;margin-left:6px;">{{ label }}</b></div>
  ...
  <div style="margin-top:10px;">
    <div style="display:flex;justify-content:space-between;font-size:11px;color:#666;margin-bottom:4px;">
      <span>Confidence Level</span><span style="color:{{ bc }};font-weight:600;">{{ lvl }}</span>
    </div>
...
function openAnnotation(imgEl, studyId) {
  ...
  // Load image onto canvas — scale to fit screen
  const img = new Image();
...
def dashboard_chat():
    ...
    system_prompt = """You are TeleRad Assistant, expert AI guide for radiology technicians.
Help with: machine operation, patient prep, scan protocols, radiation dose, troubleshooting, maintenance.
Be concise, step-by-step, beginner-friendly. Use numbered lists and emojis."""
    try:
        import requests as req
        resp = req.post(
            "https://api.groq.com/openai/v1/chat/completions",
            headers={"Authorization": f"Bearer {os.environ.get('GROQ_API_KEY')}","Content-Type":"application/json"},
            json={"model":"llama-3.3-70b-versatile","messages":[{"role":"system","content":system_prompt},{"role":"user","content":question}],"max_tokens":512,"temperature":0.7},
            timeout=30
        )
        rd = resp.json()
        reply = rd["choices"][0]["message"]["content"] if resp.status_code==200 else f"AI error: {rd.get('error',{}).get('message','Unknown')}"
    except Exception as e:
